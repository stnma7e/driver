            ...
            for i in children {
                if let Some(child) = self.inode_map.get(&i) {
                    ...
                    if path.file_name().expect(&format!("no file_name {:?}", child)) == name {
                        let size = child.size.unwrap_or_else(|| {
                            println!("no child size {:?}", child);
                            0
                        });
            ...
            if let Some(children) = self.child_map.get(&ino) {
                for child_inode in children {
                    if let Some(child) = self.inode_map.get(&child_inode) {
                        reply.add(*child_inode, *child_inode, child.kind, &child.path.file_name().expect(&format!("no file_name {:?}", child)));
                    } else {
            ...
            match File::open(&fd.path) {
                Ok(mut handle) => {
                    ...
                    match handle.read_to_end(&mut data) {
                        Ok(_) => (),
                        Err(error) => println!("couldnt read file handle, {}: error, {}", fd.path.to_string_lossy(), error),
                    };
                ...
                Err(_) => {
                    println!("no downloaded file for {}", fd.path.to_string_lossy());
                }
            }
        } else {
